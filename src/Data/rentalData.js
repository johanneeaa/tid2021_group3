...
export default async function getAllBookings() {
  ...
  const allBookingsFormatted = allBookings.map((booking) => {
    const pickUpDate = new Date(booking.get("PickUpTime"))
      .toISOString()
      .slice(5, 10); // for hours and minutes (slicing on characters from index 0 - 10)
    const pickUpTime = new Date(booking.get("PickUpTime"))
      .toISOString()
      .slice(11, 16); // for hours and minutes (slicing on characters from index 0 - 10)

    const returnDate = new Date(booking.get("ReturnTime"))
      .toISOString()
      .slice(5, 10);
    const returnTime = new Date(booking.get("ReturnTime"))
      .toISOString()
      .slice(11, 16);
